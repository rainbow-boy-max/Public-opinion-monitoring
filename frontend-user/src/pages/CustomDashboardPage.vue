  ...
  switch (type) {
    case 'sentiment_trend': {
      ...
      return {
        categories,
        series: [
          { name: '正面', data: categories.map(() => Math.floor(Math.random() * 30 + 10)) },
          { name: '负面', data: categories.map(() => Math.floor(Math.random() * 20 + 5)) },
          { name: '中性', data: categories.map(() => Math.floor(Math.random() * 25 + 8)) },
        ],
      };
    }
    case 'platform_breakdown': {
      const platforms = ['微信', '抖音', '微博', '小红书', '快手', '百家号'];
      return { data: platforms.map(name => ({ name, value: Math.floor(Math.random() * 100 + 10) })) };
    }
    case 'volume_over_time': {
      const categories: string[] = [];
      for (let i = 0; i < 24; i++) categories.push(`${String(i).padStart(2, '0')}:00`);
      return { categories, data: categories.map(() => Math.floor(Math.random() * 50 + 5)) };
    }
    case 'top_keywords': {
      const words = ['舆情', '热点', '事件', '危机', '公关', '品牌', '口碑', '投诉', '维权', '曝光',
        '举报', '监管', '政策', '行业', '市场', '竞争', '创新', '科技', '金融', '教育',
        '医疗', '房产', '汽车', '消费', '电商', '直播', '短视频', '社交', '平台', '数据'];
      return { data: words.slice(0, 20 + Math.floor(Math.random() * 10)).map(text => ({ text, count: Math.floor(Math.random() * 80 + 5) })) };
    }
    case 'recent_events': {
    ...
    case 'sentiment_gauge':
      return { value: Math.floor(Math.random() * 40 + 40), min: 0, max: 100 };
    case 'hot_topics': {
      return {
        data: Array.from({ length: 5 }, (_, i) => ({
          id: `${i}`,
          keywords: [`话题${i + 1}`, '热点'],
          currentVolume: Math.floor(Math.random() * 500 + 50),
