      ...
      <div class="query-builder">
        <div v-for="(group, idx) in queryGroups" :key="idx" class="query-group">
          ...
          <div class="query-group__fields">
            <el-input v-model="group.label" placeholder="组标签（如品牌A）" style="width: 160px" size="small" />
            <el-select
        ...
        <div class="query-filters">
          <div class="query-filters__item">
            <label class="query-filters__label">平台</label>
            <el-select v-model="selectedPlatforms" multiple placeholder="全部平台" style="width: 280px" size="small" clearable>
              <el-option v-for="p in platforms" :key="p.value" :label="p.label" :value="p.value" />
            </el-select>
          </div>
          <div class="query-filters__item">
            <label class="query-filters__label">时间范围</label>
          ...
          <div class="query-filters__item">
            <label class="query-filters__label">聚合粒度</label>
    ...
    <template v-if="hasResult">
      <GlassCard title="对比概览" padded>
        <el-table :data="overviewRows" stripe size="small" border>
          <el-table-column prop="label" label="对比组" width="180" />
          <el-table-column prop="total" label="总量" width="80" align="center" />
          <el-table-column label="正面" width="80" align="center">
            <template #default="{ row }"><span style="color:#34D399">{{ row.positive }}</span></template>
          </el-table-column>
          <el-table-column label="负面" width="80" align="center">
            <template #default="{ row }"><span style="color:#F87171">{{ row.negative }}</span></template>
          </el-table-column>
          <el-table-column label="中性" width="80" align="center">
            <template #default="{ row }"><span style="color:#94A3B8">{{ row.neutral }}</span></template>
          </el-table-column>
          <el-table-column prop="dominantPlatform" label="主要平台" width="120" align="center" />
          <el-table-column prop="avgEngagement" label="平均互动" width="100" align="center" />
        </el-table>
      </GlassCard>

      <div class="charts-grid">
        <GlassCard title="情感分布对比" subtitle="堆叠柱状图">
          <div ref="stackedEl" style="height: 300px" />
        </GlassCard>
        <GlassCard title="声量趋势对比" subtitle="折线图">
          <div ref="trendEl" style="height: 300px" />
        </GlassCard>
        <GlassCard title="平台分布雷达" subtitle="多组对比">
          <div ref="radarEl" style="height: 300px" />
        </GlassCard>
      </div>

      <GlassCard title="热门文章 Top 10" subtitle="按互动量排序">
        <el-table :data="topArticleRows" stripe size="small" style="width:100%">
          <el-table-column prop="group" label="对比组" width="140" />
          <el-table-column prop="title" label="标题" min-width="260" show-overflow-tooltip />
          <el-table-column prop="platform" label="平台" width="100" align="center" />
          <el-table-column prop="engagement" label="互动量" width="100" align="center" sortable />
        </el-table>
      </GlassCard>
    </template>
...
interface ComparisonGroupStats {
  total: number;
...
const overviewRows = computed(() => {
  ...
  return result.value.groups.map((g) => {
    ...
    return {
      label: g.label,
      total: g.stats.total,
      positive: g.stats.bySentiment.positive,
      negative: g.stats.bySentiment.negative,
      neutral: g.stats.bySentiment.neutral,
      dominantPlatform: dominant ? dominant[0] : '-',
      avgEngagement: g.stats.avgEngagement,
    };
  });
});
