...
export class BoshuChineseProvider implements ProviderImplementation {
  provider = 'boshu_chinese' as const;
    ...
    try {
      ...
      if (!resp.ok) {
        ctx.emit({
          phase: 'parse',
          level: 'warn',
          message: `boshu returned ${resp.status}`,
          ts: Date.now(),
        });
        return [];
      }
      const json: any = await resp.json();
      const items: any[] = Array.isArray(json?.data?.webPages?.value)
        ? json.data.webPages.value
        : [];
      ctx.emit({
        phase: 'parse',
        message: `${items.length} pages`,
        ts: Date.now(),
      });
      return items.slice(0, 5).map((r: any) => ({
        title: String(r.name || r.title || '').slice(0, 200),
        url: String(r.url || ''),
        snippet: String(r.snippet || r.summary || '').slice(0, 400),
        source: 'boshu_chinese',
      }));
    } finally {
