...
export class WeiboAdapter extends BaseAdapter {
  ...
  private get apiBase(): string {
    return this.configService.get<string>('WEIBO_API_BASE') || 'https://api.example.com/weibo';
  }

  private get apiKey(): string {
    return this.configService.get<string>('WEIBO_API_KEY') || '';
  }
  ...
  async fetchByKeywords(
    ...
    try {
      ...
      if (!response.ok) {
        this.logger.warn(`Weibo API returned status ${response.status}`);
        return [];
      }
      ...
      this.logger.warn(`Weibo fetch failed: ${(err as Error).message}`);
      return [];
    }
  }
