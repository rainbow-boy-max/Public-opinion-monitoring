import { Module } from '@nestjs/common';
import { TypeOrmModule } from '@nestjs/typeorm';
import { JwtModule } from '@nestjs/jwt';
import { ConfigModule, ConfigService } from '@nestjs/config';
import { LlmModelEntity } from '../../database/entities';
import { LlmModule } from '../agents/llm.module';
import { KeywordSuggestController } from './keyword-suggest.controller';
import { KeywordSuggestService } from './keyword-suggest.service';

@Module({
  imports: [
    TypeOrmModule.forFeature([LlmModelEntity]),
    JwtModule.registerAsync({
      imports: [ConfigModule],
      inject: [ConfigService],
      useFactory: (config: ConfigService) => ({
        secret: config.get<string>('JWT_SECRET') || process.env.JWT_SECRET || 'default',
      }),
    }),
    LlmModule,
  ],
  controllers: [KeywordSuggestController],
  providers: [KeywordSuggestService],
  exports: [KeywordSuggestService],
})
export class KeywordSuggestModule {}
