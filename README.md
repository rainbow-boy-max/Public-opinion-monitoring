...
### 2026-07-22 — 宝塔部署包 Phase 4：备份恢复与版本回滚
- 新增数据库备份脚本：MySQL 逻辑导出、Redis BGSAVE 快照、运行配置备份
- 新增应用回滚脚本：前端软链接原子回切、后端镜像版本回退
- 新增数据库恢复脚本：恢复前自动创建当前快照，避免误操作丢失数据
- 控制台新增「备份与回滚」Tab：备份列表、版本列表、一键备份/回滚/恢复
- `opinionctl` 新增 `backup`、`rollback`、`restore` 白名单命令
